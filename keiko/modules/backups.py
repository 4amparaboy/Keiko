...
import keiko.modules.sql.notes_sql as sql
from keiko import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP
from keiko.__main__ import DATA_IMPORT
from keiko.modules.helper_funcs.chat_status import user_admin
# from keiko.modules.rules import get_rules
import keiko.modules.sql.rules_sql as rulessql
# from keiko.modules.sql import warns_sql as warnssql
import keiko.modules.sql.blacklist_sql as blacklistsql
from keiko.modules.sql import disable_sql as disabledsql
# from keiko.modules.sql import cust_filters_sql as filtersql
# import keiko.modules.sql.welcome_sql as welcsql
import keiko.modules.sql.locks_sql as locksql
from keiko.modules.connection import connected
...
def import_data(update, context):
	...
	if msg.reply_to_message and msg.reply_to_message.document:
		try:
			...
			msg.reply_text("Try downloading and uploading the file yourself again, This one Seems broken!")
			return
...
def export_data(update, context):
	...
	f=open("keiko{}.backup".format(chat_id), "w")
	...
	try:
		...
		pass
	context.bot.sendDocument(current_chat_id, document=open('keiko{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `keiko-Backup` is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
	os.remove("keiko{}.backup".format(chat_id)) # Cleaning file
